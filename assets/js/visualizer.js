...
var formatTime = __webpack_require__(459);
...
window.Aigis = {
  ...
  tables: ['Class', 'Rarity', 'Attack', 'Heal', 'Skill', 'Ability', 'Unit', 'PrinceTitle', 'Settings'],
  data: {},
  state: {
    currentGraph: 'damage'
  },
...
MoraleEffect.prototype.dump = function () {
  ...
  return ' 攻撃力補正:' + rate + ' 防御力補正:' + rate + ' 効果時間:' + formatTime(this.duration);
};
...
EnforceStrengthEffect.prototype.dump = function () {
  return '攻撃力補正:' + (this.multiplier * 100).toFixed(1) + '%' + ' 効果時間:' + formatTime(this.duration);
};
...
EnforceHealthEffect.prototype.dump = function () {
  return '最大HP補正:' + (this.multiplier * 100).toFixed(1) + '%' + ' 効果時間:' + formatTime(this.duration);
};
...
EnforceDefenceEffect.prototype.dump = function () {
  return '防御力補正:' + (this.multiplier * 100).toFixed(1) + '%' + ' 効果時間:' + formatTime(this.duration);
};
...
AttackDelayEffect.prototype.dump = function () {
  return '攻撃ディレイ補正:' + this.cooldownDelay * 100 + '%' + ' 効果時間:' + formatTime(this.duration);
};
...
MultipleAttackEffect.prototype.dump = function () {
  return '攻撃回数:' + (this.delta > 0 ? '+' : '') + this.delta + '回' + ' 効果時間:' + formatTime(this.duration);
};
...
SelfHealingEffect.prototype.dump = function () {
  return '回復量:' + this.calcRecovery() + ' 対象最大HP:' + this.owner.status.health + ' 回復率:' + (this.multiplier * 100).toFixed(1) + '%';
};
...
EnchantAttributionEffect.prototype.dump = function () {
  var attribution = '準魔法';
  return '効果時間:' + formatTime(this.duration) + ' 属性:' + attribution;
};
...
TitleEffect.prototype.dump = function () {
  return ' HP補正:' + (Math.floor(this.title.health * 100) + 100) + '%' + ' 攻撃力補正:' + (Math.floor(this.title.strength * 100) + 100) + '%' + ' 防御力補正:' + (Math.floor(this.title.defence * 100) + 100) + '%' + ' 士気値補正:' + (Math.floor(this.title.morale * 100) + 100) + '%' + ' 効果時間:' + formatTime(this.duration);
};
...
function initialize() {
  ...
  sortKeys.forEach(function (sortKey) {
    ...
    $('<option />').val(unit.id).text(label).appendTo(group);
  });
  ...
  unitTemplateControl.select2({
    ...
    formatResult: function formatResult(option) {
      ...
      var style = 'background-position: -' + unit.iconSX + 'px -' + unit.iconSY + 'px;' + 'background-image: url(' + Aigis.settings.sprite + ');';
  ...
  $('#switch a[data-toggle=tab]').on('show.bs.tab', function (e) {}).on('shown.bs.tab', function (e) {
...
function simulate(attackers, condition) {
  ...
  units.forEach(function (attacker, index) {
    ...
    attacker.callbacks.damaged = function (elapsedTime, damage, from, before, after) {
      ...
      if (this.damage >= this.status.health) {
        ...
        var message = '攻撃対象' + this.name + 'を撃破' + ' 最大HP:' + this.status.health + ' 合計ダメージ:' + this.damage;
        data.push({
          type: 'anotation',
          time: elapsedTime,
          object: from,
    ...
    attacker.actions.heal.callbacks.warmup = function (elapsedTime) {
      ...
      var message = this.name + ' 対象:' + target.name + ' 回復量:' + recovery;
      data.push({
        type: 'anotation',
        time: elapsedTime,
        object: this.owner,
    ...
    if (condition.useSkill) {
      attacker.actions.skill.callbacks.warmup = function (elapsedTime) {
        var duration = this.getDuration();
        var message = 'スキル ' + this.name + ' を発動' + ' Lv:' + this.level;
        data.push({
          type: 'anotation',
          time: elapsedTime,
          object: this.owner,
  ...
  stage.callbacks.costChanged = function (elapsedTime, cost, from, before, after) {
    ...
    var message = 'コスト変動' + ' ' + (cost <= 0 ? '' : '+') + cost + ' (Σ' + after + ')' + ' ユニット:' + from.name;
    data.push({
      type: 'anotation',
      time: elapsedTime,
      object: this,
...
function updateGraph(attackers, condition, onCompleted, onError) {
  ...
  process([function () {
    Aigis.state.result = simulate(Aigis.state.attackers, Aigis.state.condition);
  }, function () {
    updateCurrentGraph();
  }, function () {
    onCompleted();
...
function updateDamageGraph(result, attackers, condition) {
  ...
  var columns = Array.apply(null, new Array(table.getNumberOfColumns())).map(function (v, i) {
    return i;
  });
  columns[0] = { calc: function calc(data, row) {
      return formatTime(data.getValue(row, 0), 1);
    }, type: 'string' };
...
function updateScoreGraph(result, attackers, condition) {
  ...
  var title = '撃破数 攻撃対象' + ' HP:' + condition.targetHealth + ' 防御力:' + condition.targetDefence;
...
function updateCostGraph(result, attackers, condition) {
  ...
  var title = '出撃コスト回復量 攻撃対象' + ' 防御力:' + condition.targetDefence + ' HP:' + condition.targetHealth;
  var baseline = -attackers.reduce(function (p, c) {
    return Math.max(p, c.cost);
  }, 0);
...
function toDamageDataTable(result, attackers, condition) {
  ...
  var damages = attackers.map(function () {
    return 0;
  });
  var cursor = 0;
  var damageOffset = 1;
...
function toRecoveryDataTable(result, healers, condition) {
  ...
  var costs = healers.map(function () {
    return 0;
  });
  var cursor = 0;
...
function toScoreDataTable(result, attackers, condition) {
  ...
  var scores = attackers.map(function () {
    return 0;
  });
  var cursor = 0;
  var scoreOffset = 1;
...
function updateLog(result) {
  ...
  result.forEach(function (record) {
    switch (record.type) {
      case 'anotation':
        $('<tr />').append($('<td class="time" />').text(formatTime(record.time))).append($('<td />').text(record.object.name)).append($('<td />').text(record.message)).appendTo(tbody);
        break;
    }
  });
}
...
function updateDamagePickup() {
  ...
  var indexes = selection.map(function (s) {
    return { index: s.row, time: data.getValue(s.row, 0) };
  });
...
function clearUnitList() {
  $('#unitList').data('sequence', 0).children('tbody').empty();
}
...
function addUnit(unit) {
  ...
  var row = $('<tr />').attr('data-id', id).attr('data-unit-id', unit.id).attr('data-rarity', unit.rarity.id);

  var nameControl = $('<input />').attr('type', 'text').attr('name', 'name').attr('value', unit.name).attr('class', 'form-control').attr('data-id', id);
  $('<td class="unit-name" />').append(nameControl).appendTo(row);

  $('<i class="small-icon" />').css({
    backgroundImage: 'url(' + Aigis.settings.sprite + ')',
    backgroundPosition: '-' + base.iconSX + 'px -' + base.iconSY + 'px'
  }).wrap('<td class="unit-icon" />').parent().appendTo(row);
  var td = $('<td class="unit-info" />').appendTo(row);
  var rarityControl, classControl;
  if (unit.base.editable) {
    rarityControl = $('<select />').attr('name', 'rarity').attr('class', 'form-control').attr('data-id', id);
    $.map(Aigis.data.Rarity, function (rarity) {
      $('<option />').val(rarity.id).text(rarity.name).appendTo(rarityControl);
    });
    rarityControl.val(unit.rarity.id).appendTo(td);

    classControl = $('<select />').attr('name', 'class').attr('class', 'form-control').attr('data-id', id);
    $.map(Aigis.data.Class, function (klass) {
      $('<option />').val(klass.id).text(klass.name).appendTo(classControl);
      classControl.val(unit.class.id).appendTo(td);
    });
  } else {
    rarityControl = $('<input type="hidden" />').attr('name', 'rarity').attr('data-id', id).val(unit.rarity.id).appendTo(td);
    classControl = $('<input type="hidden" />').attr('name', 'class').attr('data-id', id).val(unit.class.id).appendTo(td);
    $('<span />').addClass('label label-rarity-' + unit.rarity.id).text(unit.rarity.name).appendTo(td);
    $('<span />').addClass('label label-class').text(unit.class.name).appendTo(td);
  }

  var levelControl = $('<select />').attr('name', 'level').attr('class', 'form-control').attr('data-id', id);
  for (var i = unit.base.minLevel; i <= unit.base.maxLevel; i++) {
    $('<option />').val(i).text(i).appendTo(levelControl);
  }
  levelControl.val(unit.level);
  $('<td class="unit-lv" />').append(levelControl).appendTo(row);

  var intimacyControl = $('<select />').attr('name', 'intimacy').attr('class', 'form-control').attr('data-id', id);
  for (i = 0; i <= unit.base.maxIntimacy; i++) {
    $('<option />').val(i).text(i + '%').appendTo(intimacyControl);
  }
  intimacyControl.val(unit.intimacy || 0).attr('readonly', unit.base.maxIntimacy === 0);
  $('<td class="unit-imtimacy" />').append(intimacyControl).appendTo(row);

  var strengthControl = $('<input />').attr('type', 'text').attr('name', 'strength').attr('value', unit.strength).attr('class', 'form-control').attr('data-id', id);
  $('<td class="unit-strength" />').append(strengthControl).appendTo(row);
  ...
  */
  ...
  var attackCooldownTimeControl = $('<input />').attr('type', 'text').attr('name', 'attack-cooldown-time').attr('value', unit.attackCooldownTime / 1000).attr('class', 'form-control text-sm').attr('data-id', id);
  $('<td class="unit-attack-cooldown-time" />').append(attackCooldownTimeControl).appendTo(row);

  td = $('<td class="unit-ability" />').appendTo(row);
  var skill = unit.skill;
  var skillControl, skillLevelControl;
  if (unit.base.editable) {
    skillControl = $('<select />').attr('name', 'skill').attr('class', 'form-control').attr('data-id', id).attr('readonly', !unit.base.editable);
    $.map(Aigis.data.Skill, function (skill) {
      $('<option />').val(skill.id).text(skill.name).appendTo(skillControl);
    });
    skillControl.val(unit.skill.id).appendTo(td);

    skillLevelControl = $('<select />').attr('name', 'skill_lv').attr('class', 'form-control').attr('data-id', id).appendTo(td);
  } else {
    $('<input type="hidden" />').attr('name', 'skill').attr('data-id', id).val(skill.id).appendTo(td);
    $('<input type="hidden" />').attr('name', 'skill_lv').attr('data-id', id).val(unit.skillLevel).appendTo(td);
    var levels = $('<ul class="dropdown-menu" role="menu"></ul>');
    $('<a href="#" />').attr('data-level', 0).text('使用しない').wrap('<li>').parent().appendTo(levels);
    for (i = skill.minLevel; i <= skill.maxLevel; i++) {
      $('<a href="#" />').attr('data-level', i).text('Lv' + i).wrap('<li>').parent().appendTo(levels);
    }
    $('<div class="btn-group"><button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown"><span class="skill-label"></span> <span class="skill-level"></span> <span class="caret"></span></button></div>').append(levels).on('click', 'a', function (e) {
      e.preventDefault();
    ...
    }).appendTo(td).find('.skill-label').text(skill.name).parent().find('.skill-level').text('Lv' + unit.skillLevel);
  }

  td = $('<td class="unit-ability" />').appendTo(row);
  var abilityControl;
  if (true) {
    abilityControl = $('<select />').attr('name', 'ability').attr('class', 'form-control').attr('data-id', id);
    $.map(Aigis.data.Ability, function (ability) {
      $('<option />').val(ability.id).text(ability.name).appendTo(abilityControl);
    });
    abilityControl.val(unit.ability.id).appendTo(td).select2();
  } else {
    abilityControl = $('<input type="hidden" />').attr('name', 'ability').attr('data-id', id).val(unit.ability.id).appendTo(td);
    $('<span />').addClass('form-control-static').text(unit.ability.name).appendTo(td);
  }

  var healthControl = $('<input type="hidden" />').attr('name', 'health').val(unit.health);

  var defenceControl = $('<input type="hidden" />').attr('name', 'defence').val(unit.defence);

  var removeButton = $('<button />').attr('name', 'remove').attr('class', 'btn btn-warning btn-xs').attr('data-id', id).html('<span class="glyphicon glyphicon-remove"></span> 削除').click(function (event) {
    event.preventDefault();
    $(this).closest('tr').remove();
  });
  $('<td class="command" />').append(removeButton).append(healthControl).append(defenceControl).appendTo(row);
  ...
  if (unit.base.editable) {
    skillControl.select2().change(function () {
...
function notice(message, type) {
  type = type || 'info';
  $('<div class="alert alert-' + type + '" />').text(message).appendTo($('#notices'));
}
...
function getSharingUrl() {
  return location.protocol + '//' + location.host + location.pathname + '?' + toQueryString() + location.hash;
}
