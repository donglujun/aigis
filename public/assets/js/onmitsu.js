...
var iconSprite = __webpack_require__(!(function webpackMissingModule() { var e = new Error("Cannot find module \"url-loader?mimetype=image/png!../../resources/aigisdot/aigisdot.png\""); e.code = 'MODULE_NOT_FOUND'; throw e; }()));
...
function gameover() {
  ...
  $game.find('[data-onmitsu-object="tile"]').addClass('disabled').each(function () {
    var $tile = $(this);
...
function updateStage(level) {
  var $olds = $tileContainer.find('[data-onmitsu-object="tile"]').css('z-index', 1).addClass('fadeout');
  ...
  var $objectiveImage = $(tileImages[correctTileId]).clone().addClass('fadein').css({
    width: '64px',
    height: '64px'
  });
  ...
  tileIds.forEach(function (tileId, i) {
    ...
    $tileTemplate.clone().css({
      width: tileWidth + 'px',
      height: tileHeight + 'px',
      left: left + 'px',
      top: top + 'px'
    }).attr('data-difficulty', difficulty).data('tileId', tileId).append($(tile).clone()).appendTo($tileContainer);
  });
}

function createTileTemplate() {
  var $tpl = $('<div />').attr('class', 'onmitsu-tile').attr('data-onmitsu-object', 'tile').css('z-index', 100);
...
function createTileImages(icons, done) {
  ...
  setTimeout(function () {
    var images = icons.map(function (icon) {
      ...
      workCtx.drawImage(sprite, icon.left, icon.top, icon.width, icon.height, 0, 0, iconWidth, iconHeight);
...
function changeScene(scene, done) {
  $('[data-onmitsu-scene]').addClass('scene-hidden').hide();

  $('[data-onmitsu-scene=' + scene + ']').addClass('scene-visible').show();

  setTimeout(done, 0);
}

function showScoreLabel($trigger, time) {
  var $label = $('<span class="onmitsu-getted-score"></span>').text('+' + (Math.floor(time / 100) / 10).toLocaleString() + '秒').appendTo($tileContainer);
...
function initialize($host) {
  ...
  $game.on('click', 'button[name=play]', function (e) {
    e.preventDefault();
