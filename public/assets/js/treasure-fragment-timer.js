...
// [begin...end)
var periods = [['2015/02/05 15:00:00', '2015/02/12 10:00:00'], ['2015/02/12 15:00:00', '2015/02/19 10:00:00']].map(function (period) {
...
var maps = [{
  ...
  stamina: 12,
  expectation: 22,
  drops: [{ name: 'カケラ5', icon: 'treasure-fragment-5', set: 3 }, { name: 'カケラ3', icon: 'treasure-fragment-3', set: 2 }, { name: 'カケラ1', icon: 'treasure-fragment-1' }, { name: '虹精霊', icon: 'rainbow-sprit' }]
}, {
  name: '合成魔獣',
  charisma: 70,
  stamina: 8,
  expectation: 13,
  drops: [{ name: 'カケラ5', icon: 'treasure-fragment-5', set: 2 }, { name: 'カケラ3', icon: 'treasure-fragment-3' }, { name: 'カリオペ', icon: 'calliope' }, { name: 'ルビー', icon: 'ruby', set: 2 }]
}, {
  name: '怪しげな冒険者たち',
  charisma: 40,
  stamina: 5,
  expectation: 7,
  drops: [{ name: 'カケラ5', icon: 'treasure-fragment-5', set: 2 }, { name: 'カケラ1', icon: 'treasure-fragment-1' }, { name: 'サノスケ', icon: 'sanosuke' }, { name: '魔水晶1', icon: 'magical-crystal-1' }]
}, {
  name: '触手の脅威',
  charisma: 35,
  stamina: 3,
  expectation: 4,
  drops: [{ name: 'カケラ5', icon: 'treasure-fragment-5', set: 2 }, { name: 'カケラ1', icon: 'treasure-fragment-1' }, { name: 'バラッド', icon: 'barrad' }, { name: '金精霊', icon: 'gold-sprit' }]
}, {
  name: '大魔行列',
  charisma: 50,
  stamina: 7,
  expectation: 11,
  drops: [{ name: 'カケラ5', icon: 'treasure-fragment-5', set: 2 }, { name: 'カケラ1', icon: 'treasure-fragment-1' }, { name: 'クリストファー', icon: 'christ' }, { name: '黒精霊', icon: 'black-sprit' }]
}, {
  name: '凶魔の巣窟',
  charisma: 40,
  stamina: 4,
  expectation: 5,
  drops: [{ name: 'カケラ3', icon: 'treasure-fragment-3' }, { name: 'カケラ1', icon: 'treasure-fragment-1', set: 2 }, { name: 'アサル', icon: 'atholl' }, { name: '魔水晶3', icon: 'magical-crystal-3' }]
}, {
  name: '魔物の奇襲',
  charisma: 35,
  stamina: 2,
  expectation: 2,
  drops: [{ name: 'カケラ1', icon: 'treasure-fragment-1', set: 2 }, { name: 'ダニエラ', icon: 'daniela' }, { name: '水晶', icon: 'crystal' }]
}, {
  name: '至宝のカケラ',
  charisma: 20,
  stamina: 1,
  expectation: 1,
  drops: [{ name: 'カケラ1', icon: 'treasure-fragment-1' }]
}];

var rewards = [{ amount: 45, unit: 'gold-bucket' }, { amount: 90, unit: 'gold-sprit' }, { amount: 135, unit: 'platinum-bucket' }, { amount: 180, unit: 'gold-sprit' }, { amount: 225, unit: 'crystal-fragment' }, { amount: 270, unit: 'platinum-sprit' }, { amount: 315, unit: 'gold-bucket' }, { amount: 360, unit: 'platinum-sprit' }, { amount: 405, unit: 'platinum-bucket' }, { amount: 450, unit: 'black-sprit' }, { amount: 495, unit: 'crystal-fragment' }, { amount: 540, unit: 'black-sprit' }, { amount: 585, unit: 'gold-bucket' }, { amount: 630, unit: 'rainbow-sprit' }, { amount: 675, unit: 'platinum-bucket' }, { amount: 720, unit: 'platinum-sprit' }, { amount: 765, unit: 'crystal-fragment' }, { amount: 810, unit: 'platinum-sprit' }, { amount: 855, unit: 'gold-bucket' }, { amount: 900, unit: 'black-sprit' }, { amount: 945, unit: 'platinum-bucket' }, { amount: 990, unit: 'platinum-sprit' }, { amount: 1035, unit: 'crystal-fragment' }, { amount: 1080, unit: 'platinum-sprit' }, { amount: 1125, unit: 'gold-bucket' }, { amount: 1170, unit: 'black-sprit' }, { amount: 1215, unit: 'platinum-bucket' }, { amount: 1260, unit: 'black-sprit' }, { amount: 1305, unit: 'crystal-fragment' }, { amount: 1350, unit: 'rainbow-sprit' }, { amount: 1395, unit: 'gold-bucket' }, { amount: 1440, unit: 'platinum-sprit' }, { amount: 1485, unit: 'platinum-bucket' }, { amount: 1530, unit: 'platinum-sprit' }, { amount: 1575, unit: 'crystal-fragment' }, { amount: 1620, unit: 'black-sprit' }, { amount: 1665, unit: 'gold-bucket' }, { amount: 1710, unit: 'black-sprit' }, { amount: 1755, unit: 'platinum-bucket' }, { amount: 1800, unit: 'rainbow-sprit' }];
...
function initialize() {
  ...
  maps.forEach(function (map, i) {
    var $chart = $('<td />').attr('data-chart', i).append($('<span class="barchart" />')).append($('<span class="barchart-label" />')).append($('<span class="marathon" />'));

    $('<tr />').append($('<th />').text(map.name)).append($('<td />').text('' + map.charisma + '/' + map.stamina)).append(function () {
  ...
  if (rewardEnabled) {
    ...
    rewards.forEach(function (reward) {
      $('<tr />').attr('data-amount', reward.amount).append($('<td class="text-right" />').html('<span class="diff"></span> ' + reward.amount)).append($('<td />').html('<span class="icon icon-' + reward.unit + '"></span>')).appendTo($rewardList);
    });
  }
  ...
  maps.forEach(function (map, index) {
    $('<option />').val(index).text(map.name + ' (' + map.charisma + '/' + map.stamina + ')').appendTo($estimateMap);
  });
