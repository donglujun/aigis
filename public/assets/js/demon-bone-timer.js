...
// [begin...end)
var periods = [['2015/08/27 16:00:00', '2015/09/03 10:00:00'], ['2015/09/03 15:00:00', '2015/09/10 10:00:00']];
...
var maps = [{
  ...
  stamina: 1,
  expectation: 2,
  drops: [{ name: '魔神の骨片1', icon: 'demon-bone-1', set: 2 }, { name: '花束', icon: 'flower' }]
}, {
  name: '密林の防衛戦',
  charisma: 30,
  stamina: 2,
  expectation: 3,
  drops: [{ name: '魔神の骨片1', icon: 'demon-bone-1', set: 3 }, { name: 'フューネス', icon: 'funes' }, { name: '魔水晶', icon: 'demon-crystal-1' }]
}, {
  name: '猛追の奪還戦',
  charisma: 40,
  stamina: 4,
  expectation: 6,
  drops: [{ name: '魔神の骨片3', icon: 'demon-bone-3', set: 1 }, { name: '魔神の骨片5', icon: 'demon-bone-1', set: 3 }, { name: 'ソーマ', icon: 'soma' }, { name: 'バラッド', icon: 'barrad' }]
}, {
  name: '魔神の骨の力',
  charisma: 50,
  stamina: 7,
  expectation: 16,
  drops: [{ name: '魔神の骨片5', icon: 'demon-bone-5', set: 2 }, { name: '魔神の骨片3', icon: 'demon-bone-3', set: 2 }, { name: 'ジョバンニ', icon: 'giovanni' }, { name: '白銀の聖霊', icon: 'platinum-sprit' }]
}, {
  name: '忍者と盗賊',
  charisma: 80,
  stamina: 9,
  expectation: 15,
  drops: [{ name: '魔神の骨片5', icon: 'demon-bone-5', set: 3 }, { name: 'リカルド', icon: 'ricard' }, { name: '白バケツ', icon: 'platinum-bucket' }, { name: '魔水晶', icon: 'demon-crystal-2' }]
}, {
  name: '紅の包囲網',
  charisma: 40,
  stamina: 5,
  expectation: 6,
  drops: [{ name: '魔神の骨片3', icon: 'demon-bone-3', set: 1 }, { name: '魔神の骨片1', icon: 'demon-bone-1', set: 3 }, { name: 'カゲロウ', icon: 'kagerou' }, { name: '白バケツ', icon: 'platinum-bucket' }]
}, {
  name: '邪悪な共謀',
  charisma: 70,
  stamina: 8,
  expectation: 13,
  drops: [{ name: '魔神の骨片5', icon: 'demon-bone-5', set: 2 }, { name: '魔神の骨片3', icon: 'demon-bone-3', set: 1 }, { name: 'ルビー', icon: 'ruby' }, { name: 'ミーシャ', icon: 'misha' }]
}, {
  name: '魔に身を捧げし者',
  charisma: 90,
  stamina: 12,
  expectation: 26,
  drops: [{ name: '魔神の骨片5', icon: 'demon-bone-5', set: 4 }, { name: '魔神の骨片3', icon: 'demon-bone-3', set: 2 }, { name: '虹の聖霊', icon: 'rainbow-sprit' }]
}, {
  name: 'ピピンの決心',
  charisma: 100,
  stamina: 2,
  expectation: 0,
  drops: [{ name: '金の聖霊', icon: 'gold-sprit' }, { name: '白の聖霊', icon: 'platinum-sprit' }, { name: '黒の聖霊', icon: 'black-sprit' }, { name: '虹の聖霊', icon: 'rainbow-sprit' }]
}];
...
function initialize() {
  ...
  $('#estimate_bug').on('click', 'button', function (e) {
    state.version++;
    saveState(state);
    ...
    $(e.delegateTarget).each(function () {
      ...
      if (animationSupporeted) {
        $this.addClass('animated hinge').one(animationEndEventName, function () {
          $this.hide();
        });
      } else {
        $this.hide();
  ...
  }).toggle(state.version === 1).each(function () {
    var $tutorial = $(this);
    var $anna = $tutorial.find('.anna');
    $tutorial.on('mouseenter', function () {
      $anna.addClass('animated bounce');
